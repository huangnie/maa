...
class Logger
{
    ...
    */
    ...
    {
        $folder = strlen($folder) > 1 ? implode(DIRECTORY_SEPARATOR, preg_split('/\\\//', $folder)) .DIRECTORY_SEPARATOR : '';
        self::$errorFile = LOG_PATH . DIRECTORY_SEPARATOR . $folder . self::$errorFile;
        $dir = dirname(self::$errorFile);
    ...
    */
    ...
    {
        if(defined('DEGUG') && DEGUG === true)  echo "<p>An error occured, The error has been reported: {$msg}.</p>";
        exit;
    }
    ...
    */
    public static function newMessage($exception)
    {
        ...
        self::setFolder();  
        if(defined('DEGUG') && DEGUG === true) echo $logMessage ; 
